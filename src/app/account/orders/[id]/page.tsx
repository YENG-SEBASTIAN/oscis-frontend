...
export default function OrderDetailPage() {
  ...
  if (loadingOrder) return <div className="max-w-5xl mx-auto px-4 py-10 space-y-6"><Skeleton className="h-20 w-full" /><Skeleton className="h-40 w-full" /><Skeleton className="h-32 w-full" /><Skeleton className="h-24 w-full" /></div>;
  if (error) return <div className="min-h-screen flex items-center justify-center text-red-600 font-semibold text-lg">Failed to load order: {error}</div>;
  if (!order) return (
    <div className="min-h-screen flex flex-col items-center justify-center text-center px-6">
      ...
      <Link href="/account/orders" className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10 space-y-8">
      ...
      <div className="bg-white rounded-xl shadow p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border border-gray-200">
        <div>
          <Link href="/account/orders" className="inline-flex items-center text-blue-600 hover:text-blue-700 text-sm font-medium mb-2">
        ...
        <div className="flex items-center gap-4">
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-md font-bold text-white ${order.order_status === 'Cancelled' ? 'bg-red-600' : 'bg-blue-600'}`}>
      ...
      <div className="bg-white rounded-xl shadow p-6 grid grid-cols-1 lg:grid-cols-2 gap-6 border border-gray-200">
        <div>
          ...
          <div className="space-y-3 text-md">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-black">{AppSettings.currency}{order.total_price}</span>
            </div>
            <div className="flex justify-between border-t pt-3 font-bold">
              <span className="text-black">Paid</span>
              <span className="text-blue-600">{AppSettings.currency}{order.payment.amount}</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="p-4 bg-blue-50 rounded-lg">
            ...
            <p className="text-sm text-blue-600 font-semibold">Status: {order.payment.status}</p>
          </div>
          <div className="p-4 bg-blue-50 rounded-lg">
            <div className="flex items-center gap-2 mb-1">
          ...
          {order.items.map((item) => (
            <div key={item.id} className="flex gap-4 p-4 border rounded-lg">
              <div className="w-20 h-20 bg-gray-100 rounded overflow-hidden flex-shrink-0">
                <Image src={item.product_image || "/placeholder.png"} alt={item.product_name} width={80} height={80} className="w-full h-full object-cover" />
              </div>
              <div className="flex-1">
                <h4 className="text-md font-bold text-black">{item.product_name}</h4>
                <p className="text-sm text-black">Qty: {item.quantity}</p>
                <p className="text-sm text-black">Price: {AppSettings.currency}{item.price}</p>
              </div>
              <div className="text-right">
                <p className="font-semibold text-black">{AppSettings.currency}{item.total_price}</p>
                <a href='/products' className="mt-2 inline-flex items-center text-blue-600 hover:text-blue-700 text-sm font-medium">
      ...
      {/* Vertical Tracking Timeline with User Action */}
      <div className="bg-white rounded-xl shadow p-6 border border-gray-200 flex flex-col lg:flex-row gap-6">
        <div className="flex-1">
          ...
          <div className="flex flex-col relative">
            {statusSteps.filter(s => s.key !== 'Cancelled' || order.order_status === 'Cancelled').map((step, index, arr) => {
              const stepIndex = statusSteps.findIndex(s => s.key === step.key);
              const isCompleted = stepIndex <= currentStepIndex;
              const isCancelledStep = step.key === 'Cancelled' && order.order_status === 'Cancelled';

              return (
                <div key={step.key} className="flex items-start relative mb-6 last:mb-0">
                  {index < arr.length - 1 && (
                    <div className={`absolute top-10 left-5 w-1 h-full ${stepIndex < currentStepIndex ? 'bg-blue-600' : 'bg-gray-300'}`} />
                  )}
                  <div className={`flex items-center justify-center w-10 h-10 rounded-full z-10 ${isCompleted ? isCancelledStep ? 'bg-red-600 text-white' : 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}>
                    {step.icon}
                  </div>
                  <div className="ml-4">
                    <span className={`text-sm font-semibold ${isCompleted ? isCancelledStep ? 'text-red-600' : 'text-blue-600' : 'text-gray-600'}`}>
                      {step.label}
                    </span>
                  </div>
                </div>
        ...
        <div className="w-48 self-start">
          <h3 className="text-md font-bold mb-2 text-black">User Action</h3>
          <select
            ...
            disabled={!canCancel || order.order_status === 'Cancelled'}
            defaultValue=""
            onChange={(e) => e.target.value === 'Cancelled' && updateOrder(order.id, { order_status: 'Cancelled' })}
          >
            <option value="" disabled>Select Action</option>
            {canCancel && order.order_status !== 'Cancelled' && <option value="Cancelled">Cancel Order</option>}
          </select>
          {!canCancel && <p className="text-xs text-gray-500 mt-1">Orders can only be cancelled within 24 hours</p>}
        </div>
      </div>
